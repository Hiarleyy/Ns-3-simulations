...
void CalculateThroughput(Ptr<FlowMonitor> monitor, Ptr<FlowClassifier> classifier) {
  FlowMonitor::FlowStatsContainer stats = monitor->GetFlowStats();

  for (auto it = stats.begin(); it != stats.end(); ++it) {
    FlowId flowId = it->first;
    FlowClassifier::FlowKey flowKey;
    classifier->GetFlowKeyFromFlowId(flowId, flowKey);

    // Extrair as informações relevantes da tupla do fluxo
    Ipv4Address sourceAddress = flowKey.sourceAddress;
    Ipv4Address destinationAddress = flowKey.destinationAddress;
    uint16_t sourcePort = flowKey.sourcePort;
    uint16_t destinationPort = flowKey.destinationPort;
    uint8_t protocol = flowKey.protocol;

    double receivedBytes = it->second.rxBytes;
    double endTime = it->second.timeLastRxPacket.GetSeconds();
    double startTime = it->second.timeFirstTxPacket.GetSeconds();

    double flowThroughput = receivedBytes * 8 / (endTime - startTime) / 1e6;
